    ...
    <el-dialog :title="dialogTitle" width="500px" :visible.sync="dvEdit">
      <el-form ref="form" :model="form" label-width="60px">
        <el-form-item label="IP">
          <el-input v-model="form.ip" @keyup.enter.native="onSubmit"></el-input>
        </el-form-item>
...
<script>
  import {getIpConfigListApi, getIpConfigDelApi,getIpStatusApi,
  getIpConfigEditApi,getIpConfigAddApi} from '@/api/IpConfigApi'
  export default {
    data () {
      return {
        dvEdit: false,
    ...
    methods: {
     ...
     onSubmit() {
       if (this.form.id) {
         ...
         getIpConfigEditApi(this.form).then(res => {
           if(res.code == 1){
            this.dvEdit = false
       ...
       this.form.status=1;
       getIpConfigAddApi(this.form).then(res => {
         if(res.code == 1){
          this.dvEdit = false
          this.$message(res.msg);
          this.getUserList();
      ...
      deleteUser(scope) {
        this.$confirm('您确定要删除当前用户吗', '提示', {
          ...
          getIpConfigDelApi({'id':scope.id}).then(res=>{
                if(res.code == 1){
