   ...
   <div slot="top">
       ...
       <el-row :gutter="20">
         ...
         <el-col :xs="24" :sm="12" :lg="1">
                 <el-date-picker
                   ...
                   type="date"
				   format="yyyy-MM-dd HH:mm:ss"   @change="handleEndDate()"  @keydown.enter.capture.native="handleEndDate()"
                   placeholder="结束日期">
                 </el-date-picker>
         </el-col>
    ...
    <el-table style="margin-top: 15px" border :data="roleList" :showPage="false">
      ...
      <el-table-column label="累积充值" width="100" prop="money_total_recharge" >
        <template slot-scope="scope">
          <el-button type="primary" @click="showRechargeList(scope.row,101)">{{scope.row.money_total_recharge}}</el-button>
        </template>
      </el-table-column>
      <el-table-column label="累计提现" width="100" prop="money_total_withdraw" >
        <template slot-scope="scope">
          <el-button type="primary" @click="showRechargeList(scope.row,201)">{{scope.row.money_total_withdraw}}</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column label="虚拟账号" width="80" prop="is_fictitious">
        <template slot-scope="scope">
          ...
          <el-switch   v-model="scope.row.is_fictitious" @change="changes(scope.row.is_fictitious,scope.row)" :active-value="1" :inactive-value="0"></el-switch>
        </template>
      </el-table-column>
      ...
      <el-table-column label="推广地址" width="120" prop="tg_url_txt" />
...
<script>
  import {getUserStatusApi,getIsStatusApi,getUserListApi,getUserDelApi,getUserEditApi,getUserAddApi,getMoneyEditApi} from '@/api/userApi'
