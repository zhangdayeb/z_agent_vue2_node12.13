<template>
  <div class='page-content'>
    <el-row :gutter="20">
      ...
      <el-col :span="6">
        <el-date-picker v-model="searchParameter.end" @keydown.enter.capture.native="search" format="yyyy-MM-dd HH:mm:ss" @change="handleEndDate()"  type="date" placeholder="结束日期">
        </el-date-picker>
      </el-col>
      <el-col :span="2">
      ...
      <el-col :span="1" class="el-col2">
        <el-button v-if="count_money != 0 " type="warning" plain>下注盈亏：{{count_money}}</el-button>
      </el-col>
    </el-row>
...
<script>
import {getRecordListApi} from '@/api/MoneylogApi'
...
export default {
  ...
  created(){
    ...
    if(dateType == 'today') {
      ...
      this.searchParameter.end =this.$minTool.handleDateType[dateType].start
    }
  },
  mounted() {
    this.getUserList()
  ...
  methods: {
    ...
    getRecordListApi:function(index){
      this.current = index;
    ...
    getUserList(){
      getRecordListApi({create_time_sort:this.create_time_sort,start: this.searchParameter.start,
